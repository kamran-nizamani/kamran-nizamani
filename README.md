...
```mermaid
flowchart LR
    A[Client / UI<br/>React + Tailwind] --> B[API Layer<br/>Express + Middleware]
    B --> C[Business Logic<br/>Validation & Auth]
    C --> D[(Database<br/>MySQL / MongoDB)]
    B -.->|JWT / OAuth 2.0| E[Auth Service]
    C -.->|Cache| F[(Redis / In-memory)]
```
...
```mermaid
flowchart TD
    A[Data Collection<br/>APIs / CSV / Scraping] --> B[Preprocessing<br/>Cleaning & Encoding]
    B --> C[EDA<br/>Stats & Visualization]
    C --> D[Feature Engineering]
    D --> E[Model Training<br/>scikit-learn / XGBoost]
    E --> F[Evaluation<br/>Cross-validation]
    F --> G[Deployment<br/>API / Serialization]
```
